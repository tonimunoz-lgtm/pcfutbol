...
function simulateFullWeek() {
    ...
    if (gameState.seasonType === 'preseason') {
        ...
        if (gameState.week > PRESEASON_WEEKS) {
            ...
            gameState.week = 1;
            addNews(
                `¡Comienza la temporada regular ${gameState.currentSeason} en ${gameState.division}!`,
                'success'
            );
        }
    ...
    gameState.squad.forEach(p => {
        ...
        if (p.isSuspended) {
            ...
            if (p.suspensionWeeks <= 0) {
                p.isSuspended = false;
                p.suspensionWeeks = 0;
                addNews(`✅ ${p.name} ha cumplido su sanción.`, 'success');
            }
        }
    });
    ...
    const currentWeekMatches = gameState.seasonCalendar.filter(
        m => m.week === gameState.week
    );

    let myTeamMatch = currentWeekMatches.find(
        m => m.home === gameState.team || m.away === gameState.team
    );
    ...
    if (myTeamMatch) {
        if (!preSimLineupValidation.success) {
            addNews(
                '[SISTEMA] Derrota 0-3 por alineación indebida.',
                'error'
            );
    ...
    // ---------------------------
    currentWeekMatches
        .filter(m => m !== myTeamMatch)
        .forEach(match => {
            const alreadyPlayed = gameState.matchHistory.some(
                mh => mh.week === gameState.week &&
                      mh.home === match.home &&
                      mh.away === match.away
            );
    ...
    if (
        gameState.staff.segundoEntrenador &&
        gameState.weeklyIncome - gameState.weeklyExpenses < -10000 &&
        gameState.balance < 0
    ) {
        addNews(
            '[Segundo Entrenador] ¡Crisis económica grave!',
            'error'
        );
    }
    ...
    if (gameState.balance < -100000 && gameState.week > 10) {
        addNews(
            '¡Has sido despedido por mala gestión económica!',
            'error'
        );
        alert('GAME OVER');
        resetGame();
