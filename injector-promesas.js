...
(function () {
    ...
    // ============================================================
    // PASO 12: Parchar deals financieros para Liga Promesas
    //   - Cantidades muy pequeñas (liga escolar de formación)
    //   - TV: cadenas locales del Vallès Occidental
    //   - Sponsor: empresas locales de la zona
    //   - Sin Copa del Rey ni competiciones europeas
    // ============================================================
    function patchFinancialDeals() {
        function tryPatch() {
            // Esperamos a que injector-financial-deals haya cargado su boot
            if (!window._finDealsBooted && !window._fdAccept) {
                setTimeout(tryPatch, 700);
                return;
            }

            // ── 1. Añadir DEAL_BASE para promesas ─────────────────
            // El objeto DEAL_BASE es local al closure de injector-financial-deals,
            // pero generateOffers() llama a getTvCompanies(div) y getSponsorCompanies(div).
            // Interceptamos esas funciones si están expuestas, o bien hookeamos
            // generateOffers vía el sistema de ofertas.
            // La forma más limpia: monkey-patch window._fdShowOffers para
            // sustituir las empresas cuando la división es promesas.

            // Interceptar el generador de ofertas que hay en window
            // El injector-financial-deals expone window._fdAccept, _fdReject, _fdShowOffers
            // pero no expone generateOffers directamente.
            // Usamos un MutationObserver sobre el modal de ofertas para reemplazar
            // empresas y montos ANTES de que se muestre.

            // ── 2. Hook sobre maybeGenerateOffers ─────────────────
            // La forma más robusta: interceptar simulateWeek para que,
            // cuando la división sea promesas, inyectemos las ofertas locales
            // DESPUÉS de que el sistema original las genere.
            ...
            // Aplicar inmediatamente si ya estamos en promesas
            const state = window.gameLogic?.getGameState();
            if (state?.division === 'promesas') {
                overridePromasasDeals();
            }

            // ── 3. Bloquear Copa del Rey en Promesas ──────────────
    ...
    // Cadenas de TV locales del Vallès Occidental y entorno
    const PROMESAS_TV = [
    ...
    const PROMESAS_SPONSORS = [
        ...
        { name: 'Clinica Fisio Sant Cugat',   sector: 'Salut' },
        ...
        { name: 'Assegurances Mutua Terrassa', sector: 'Assegurances' },
    ];

    function overridePromasasDeals() {
        // Leer el estado de deals del localStorage (clave usada por injector-financial-deals)
        ...
        // Montos acordes a una liga escolar de formación
        // Sponsor: 500-2.000€/año  |  TV: 200-800€/año
    ...
    function blockCopaForPromesas() {
        // Interceptar initCompetitionsForSeason del sistema de competiciones
        // para que en promesas no se genere Copa ni Europa
        if (!window.CompetitionsSystem) return;

        const origReset = window.CompetitionsSystem.reset;
        if (origReset && !window._promasasCopaBlocked) {
            window._promasasCopaBlocked = true;
            window.CompetitionsSystem.reset = function() {
                const state = window.gameLogic?.getGameState();
                if (state?.division === 'promesas') {
                    // En promesas: limpiar comp pero NO inicializar Copa ni Europa
                    // Solo reiniciamos el playoff de promesas
                    storeP.clear();
                    console.log('✅ Promesas: Copa y Europa bloqueadas');
                    return;
                }
        ...
        // También interceptar processWeeklyComps para bloquear Copa en promesas
        // (el hook de simulateWeek del injector-competitions llama a processWeeklyComps)
        // Lo hacemos neutralizando el tab de Copa en la UI de clasificación
        setTimeout(() => {
            const state = window.gameLogic?.getGameState();
            if (state?.division === 'promesas') {
