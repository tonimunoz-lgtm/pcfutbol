...
function renderStandingsTable(state) {
    ...
    standingsDiv.innerHTML = rowsHtml;
}
...
function renderSquadList(squad, currentTeam) {
    ...
    if (!squad || squad.length === 0) {
        list.innerHTML = '<div class="alert alert-info">No hay jugadores en la plantilla</div>';
        return;
    }
            ...
            <tr style="${p.club === currentTeam ? 'background: rgba(233, 69, 96, 0.1);' : ''}">
                ...
                <td style="display: flex; gap: 3px; flex-wrap: nowrap; justify-content: center;">
                    ${p.contractType !== 'loaned_out' ? `
                        <button class="btn btn-sm" onclick="window.openTrainingModal(${idx})" title="Entrenar" style="padding: 5px 8px;">💪</button>
                    ` : ''}
                    ${contractType === 'owned' ? `
                        ${!p.transferListed && !p.loanListed ? `
                            <button class="btn btn-sm" style="background: #FF9800; padding: 5px 8px;" onclick="window.openSellPlayerModal(${idx})" title="Poner en venta">💰</button>
                        ` : `
                            <button class="btn btn-sm" style="background: #9E9E9E; padding: 5px 8px;" onclick="window.removeFromMarket(${idx})" title="Retirar del mercado">❌</button>
                        `}
                        <button class="btn btn-sm" style="background: #c73446; padding: 5px 8px;" onclick="window.firePlayerConfirm('${p.name}')" title="Despedir">🚪</button>
                    ` : p.contractType === 'loaned_out' ? `
                        <span style="color: #9E9E9E; font-size: 0.85em;">📤 Cedido</span>
                    ` : `
                        <button class="btn btn-sm" style="background: #9E9E9E; padding: 5px 8px;" disabled title="Cedido a nosotros">🔒</button>
                    `}
                </td>
            </tr>
        `;
    ...
    let playersHtml = players.map((p, idx) => {
        ...
        return `
            <tr>
                ...
                <td style="color: #FF9800; font-weight: bold;">${clauseAmount.toLocaleString('es-ES')}€</td>
                <td>${askingPrice}</td>
                <td>${estado}</td>
                <td>
                    ${p.transferListed || p.loanListed ? `
                        <button class="btn btn-sm" onclick="window.startNegotiationUI('${encodeURIComponent(JSON.stringify(p))}')">Negociar</button>
                    ` : `
                        <button class="btn btn-sm" style="background: #FF5722;" onclick="window.payReleaseClause('${encodeURIComponent(JSON.stringify(p))}')">💰 Cláusula</button>
                    `}
                </td>
            </tr>
        `;
